...
class pincell():
    ...
    def create_materials(self):
        ...
        fuel_No_BA.volume = 3.14*(self.pin_fuel_or**2)
        ...
        # water
        H2O = openmc.model.borated_water(boron_ppm=2000, density=0.742)
        H2O.add_s_alpha_beta('c_H_in_H2O')

        #openmc object to export
        self.materials = openmc.Materials([M5, He, H2O, fuel_No_BA])
    ...
    def create_geometry(self):
        ...
        #boundaries
        left = openmc.XPlane(x0=-self.pitch/2, boundary_type='reflective')
        right = openmc.XPlane(x0=self.pitch/2, boundary_type='reflective')
        bottom = openmc.YPlane(y0=-self.pitch/2, boundary_type='reflective')
        top = openmc.YPlane(y0=self.pitch/2, boundary_type='reflective')

        #FUEL_No_BA PIN UNIVERSE
        fuel_No_BA_cell = openmc.Cell(fill=self.fuel_No_BA, region=-pin_fuel_or, cell_id=5)
        gap_No_BA_cell = openmc.Cell(fill=self.He, region= +pin_fuel_or & -pin_clad_ir, cell_id=6)
        clad_No_BA_cell = openmc.Cell(fill=self.M5, region= +pin_clad_ir & -pin_clad_or, cell_id=7)
        water_No_BA_cell = openmc.Cell(fill=self.H2O, region= +pin_clad_or & +left & -right & +bottom & -top, cell_id=8)

        f = openmc.Universe(cells=[fuel_No_BA_cell, gap_No_BA_cell, clad_No_BA_cell, water_No_BA_cell], name='f')

        self.geometry = openmc.Geometry(root=f)
    ...
    def plot_geometry(self):
        ...
        geom_plot_top.width = (self.pitch, self.pitch)
        geom_plot_top.pixels = (1500, 1500)
        geom_plot_top.color_by = 'material'
        geom_plot_top.colors = {self.fuel_No_BA: (255,255,153), self.He:'green', self.M5:'grey', self.H2O:(102,178,255)}
        ...
        geom_plot_side.width = (self.pitch, self.pitch)
        geom_plot_side.pixels = (1500, 1500)
        geom_plot_side.color_by = 'material'
        geom_plot_side.colors = {self.fuel_No_BA: (255,255,153), self.He:'green', self.M5:'grey', self.H2O:(102,178,255)}
...
class assembly():
    ...
    def create_materials(self):
        ...
        # water
        H2O = openmc.model.borated_water(boron_ppm=0, density=0.742)
        H2O.add_s_alpha_beta('c_H_in_H2O')
    ...
    def create_geometry(self, config):
        ...
        fuel_BA_cell = openmc.Cell(fill=self.fuel_BA, region=-pin_fuel_or, cell_id=1)
        gap_BA_cell = openmc.Cell(fill=self.He, region= +pin_fuel_or & -pin_clad_ir, cell_id=2)
        clad_BA_cell = openmc.Cell(fill=self.M5, region= +pin_clad_ir & -pin_clad_or, cell_id=3)
        water_BA_cell = openmc.Cell(fill=self.H2O, region= +pin_clad_or, cell_id=4)

        b = openmc.Universe(cells=[fuel_BA_cell, gap_BA_cell, clad_BA_cell, water_BA_cell], name='b')

        #FUEL_No_BA PIN UNIVERSE
        fuel_No_BA_cell = openmc.Cell(fill=self.fuel_No_BA, region=-pin_fuel_or, cell_id=5)
        gap_No_BA_cell = openmc.Cell(fill=self.He, region= +pin_fuel_or & -pin_clad_ir, cell_id=6)
        clad_No_BA_cell = openmc.Cell(fill=self.M5, region= +pin_clad_ir & -pin_clad_or, cell_id=7)
        water_No_BA_cell = openmc.Cell(fill=self.H2O, region= +pin_clad_or, cell_id=8)

        f = openmc.Universe(cells=[fuel_No_BA_cell, gap_No_BA_cell, clad_No_BA_cell, water_No_BA_cell], name='f')

        #CHANNEL UNIVERSE
        in_channel_cell = openmc.Cell(fill=self.H2O, region=-channel_clad_ir, cell_id=9)
        channel_clad_cell = openmc.Cell(fill=self.M5, region= +channel_clad_ir & -channel_clad_or, cell_id=10)
        out_channel_cell = openmc.Cell(fill=self.H2O, region=+channel_clad_or, cell_id=11)

        c = openmc.Universe(cells=[in_channel_cell, channel_clad_cell, out_channel_cell], name='c')

        #OUTER UNIVERSE
        all_water_cell = openmc.Cell(fill=self.H2O, cell_id=12)
